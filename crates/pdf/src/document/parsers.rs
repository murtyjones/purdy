...
fn maybe_parse_object(
    (input, (_id, entry)): (&'static [u8], (&u32, &XrefEntry)),
...
pub fn make_xref_table_from_end_of_file(input: &'static [u8]) -> Result<(Xref, Dictionary)> {
    ...
    let starting_search_pos_startxref = input.len().saturating_sub(1024);
...
pub fn make_xref_table_manually(input: &'static [u8]) -> Result<(Xref, Dictionary)> {
    let mut xref_table = Xref::default();
...
fn assert_all_xref_entry_offsets_are_accurate(input: &[u8], xref: &Xref) -> Result<()> {
...
fn xref(input: &[u8]) -> NomResult<Xref> {
    ...
    let (input, _number_of_objects) = ws(int1::<u32>)(input)?;
...
fn hex_char1(input: &[u8]) -> NomResult<u8> {
    map_res(
        ...
        |x: &[u8]| u8::from_str_radix(from_utf8(&[x[0], 48]).unwrap(), 16),
    )(input)
}
...
mod tests {
    ...
    #[test]
    fn test_real() {
        assert_relative_eq!(strip(_real::<f64>(b"100.4292498")), 100.4292498);
        assert_relative_eq!(strip(_real::<f64>(b"+100.4292498")), 100.4292498);
        assert_relative_eq!(strip(_real::<f64>(b"-100.4292498")), -100.4292498);
        assert_relative_eq!(strip(_real::<f64>(b"-.4292498")), -0.4292498);
        assert_relative_eq!(strip(_real::<f64>(b"-0.4292498")), -0.4292498);
        assert_relative_eq!(strip(_real::<f64>(b"-0.4292498")), -0.4292498);
        assert_relative_eq!(strip(_real::<f64>(b"34.5")), 34.5);
        assert_relative_eq!(strip(_real::<f64>(b"-3.62")), -3.62);
        assert_relative_eq!(strip(_real::<f64>(b"-123.6")), -123.6);
        assert_relative_eq!(strip(_real::<f64>(b"4.")), 4.0);
        assert_relative_eq!(strip(_real::<f64>(b"-.002")), -0.002);
        assert_relative_eq!(strip(_real::<f64>(b"0.0")), 0.0);
    }
